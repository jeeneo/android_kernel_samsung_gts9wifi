...
{pkgs ? import <nixpkgs> {}}: let
  ...
  fhs = pkgs.buildFHSEnv {
    ...
    profile = ''
      ...
      # the toolchain is grossly incompatible with NixOS currently and im too lazy to fix, so just disable
      ...
      # Force clang to keep /usr/include on its search path even when the
      # Android build prepends a host-prebuilt GCC to PATH whose sysroot has
      # no sys/types.h (otherwise host tools like scripts/basic/fixdep fail).
      # CPATH/C_INCLUDE_PATH are honored by clang for both C and C++.
      ...
      export HOSTCFLAGS="--sysroot=$HOST_LLVM_SYSROOT -Wno-unused-command-line-argument"
